...
max_temp_diff_per_month = temp[:, :, 1:, 1] - temp[:, :, :-1, 1]

print("Temperature Difference Between Max Temp of Consecutive Days for Each Month:")
print(max_temp_diff_per_month)
...
min_temp_diff_per_month = temp[:, :, 1:, 0] - temp[:, :, :-1, 0]
...
#25
temp_diff_per_day_all_months = temp[:, :, :, 1] - temp[:, :, :, 0]

# Combine the arrays from q.23 and q.24
combined_temp_diff = np.stack((min_temp_diff_per_month, max_temp_diff_per_month), axis=3)

print("Combined Difference Between Min and Max Temp for Each Day of All Months:")
